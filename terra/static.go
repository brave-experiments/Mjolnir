...
resource "aws_iam_policy" "bastion" {
  name        = "pantheon-bastion-policy-${var.network_name}"
  path        = "/"
...
do
  ...
  HOST_IP=$(echo $task_metadata | jq -r '.tasks[0] | .containers[] | select(.name == "${local.pantheon_run_container_name}") | .networkInterfaces[] | .privateIpv4Address')
  if [ "${var.ecs_mode}" == "EC2" ]
  then
...
resource "aws_security_group" "bastion-ssh" {
  vpc_id      = "${local.vpc_id}"
  name        = "pantheon-bastion-ssh-${var.network_name}"
  description = "Security group used by Bastion node to access pantheon network ${var.network_name}"
  ...
  tags = "${merge(local.common_tags, map("Name", format("pantheon-bastion-ssh-%s", var.network_name)))}"
}

resource "aws_security_group" "bastion-ethstats" {
  vpc_id      = "${local.vpc_id}"
  name        = "pantheon-bastion-ethstats-${var.network_name}"
  description = "Security group used by external to access ethstats for pantheon network ${var.network_name}"
