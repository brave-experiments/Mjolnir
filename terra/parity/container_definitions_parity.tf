locals {
  ...
  parity_config_commands = [
    ...
    "all=\"\"; for f in $(ls ${local.node_ids_folder}); do nodeid=$(cat ${local.node_ids_folder}/$f); ip=$(cat ${local.hosts_folder}/$f); echo \"enode://$nodeid@$ip:${local.parity_p2p_port}\"; done; >> ${local.parity_static_nodes_file}",
    "#echo $all > ${local.parity_static_nodes_file}",
