...
do
  ...
  i=$(($i+1))
    if [ $i -eq 1 ]; then
      sed -i s"/^RPCaddress=.*/RPCaddress=\'http:\/\/$ip:${local.local.parity_rpc_port}\'/"   $WORKDIR/hammer/config.py
    elif [ $i -eq 2 ]; then
      sed -i s"/^RPCaddress2=.*/RPCaddress2=\'http:\/\/$ip:${local.local.parity_rpc_port}\'/" $WORKDIR/hammer/config.py
    elif [ $i -gt 2 ]; then
      continue
    fi
done
